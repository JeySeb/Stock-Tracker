...
func (r *SessionRepositoryImpl) Create(ctx context.Context, session *sessionModel.Session) error {
	query := `
...
func (r *SessionRepositoryImpl) GetByRefreshToken(ctx context.Context, refreshToken string) (*sessionModel.Session, error) {
	query := `
...
func (r *SessionRepositoryImpl) GetByUserID(ctx context.Context, userID uuid.UUID) ([]*sessionModel.Session, error) {
	query := `
		SELECT id, user_id, refresh_token, user_agent, ip_address, expires_at, created_at
		FROM sessions 
