...
func TestNewMarketData(t *testing.T) {
	...
	externalData := &ExternalStockData{
		CurrentPrice:     100.50,
		DayChange:        5.25,
	...
	// Test NewMarketData
	marketData := NewMarketData("AAPL", DataSourceYahooFinance, externalData)

	// Assertions
	assert.NotNil(t, marketData)
	assert.NotEqual(t, uuid.Nil, marketData.ID)
	assert.Equal(t, "AAPL", marketData.Ticker)
	assert.Equal(t, DataSourceYahooFinance, marketData.DataSource)
	assert.Equal(t, DataQualityGood, marketData.DataQuality)
...
func TestNewMarketDataIngestionLog(t *testing.T) {
	...
	log := NewMarketDataIngestionLog(batchID, DataSourceYahooFinance, totalTickers)

	// Assertions
	assert.NotNil(t, log)
	assert.NotEqual(t, uuid.Nil, log.ID)
	assert.Equal(t, batchID, log.BatchID)
	assert.Equal(t, DataSourceYahooFinance, log.DataSource)
...
func TestConvertFromRecommendationModel(t *testing.T) {
	...
	// Test conversion
	marketData := ConvertFromRecommendationModel("MSFT", DataSourceYahooFinance, recModel)
...
func TestConvertFromRecommendationModel_ZeroMarketCap(t *testing.T) {
	...
	// Test conversion
	marketData := ConvertFromRecommendationModel("TEST", DataSourceYahooFinance, recModel)
...
func TestDataSourceConstants(t *testing.T) {
	...
	assert.Equal(t, DataSource("yahoo_finance"), DataSourceYahooFinance)
	assert.Equal(t, DataSource("alpha_vantage"), DataSourceAlphaVantage)
	assert.Equal(t, DataSource("manual"), DataSourceManual)
}

func TestDataQualityConstants(t *testing.T) {
	// Test DataQuality constants
	assert.Equal(t, DataQuality("excellent"), DataQualityExcellent)
	assert.Equal(t, DataQuality("good"), DataQualityGood)
	assert.Equal(t, DataQuality("fair"), DataQualityFair)
	assert.Equal(t, DataQuality("poor"), DataQualityPoor)
}
...
func TestMarketData_JSONTags(t *testing.T) {
	...
	marketData := &MarketData{
		ID:               uuid.New(),
		Ticker:           "TEST",
		DataSource:       DataSourceYahooFinance,
