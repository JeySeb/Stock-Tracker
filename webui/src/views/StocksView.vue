<template>
  <div class="space-y-6">
    <!-- Header -->
    <div class="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 text-white mb-6">
      <div class="text-center">
        <h1 class="text-2xl font-bold">Stock Events</h1>
    ...
    <!-- Stats Row -->
    <div v-if="stocksStore.stats" class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
      <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <div class="flex items-center">
          <div class="flex-shrink-0">
            <div class="w-8 h-8 bg-primary-100 rounded-full flex items-center justify-center">
              <span class="text-primary-600 font-semibold text-sm">{{ formatNumber(stocksStore.stats.total_stocks) }}</span>
            </div>
          </div>
          <div class="ml-3 min-w-0 flex-1">
            <p class="text-xs font-medium text-gray-500">Total Stock Events</p>
            <p class="text-lg font-semibold text-gray-900">{{ stocksStore.stats.total_stocks.toLocaleString() }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <div class="flex items-center">
          ...
          <div class="ml-3 min-w-0 flex-1">
            ...
            <p class="text-lg font-semibold text-gray-900">{{ formatDate(stocksStore.stats.last_updated) }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="mb-6">
      <StockFilters
        ...
        :items-per-page="stocksStore.itemsPerPage"
    ...
    <DataTable
      ...
      :pagination="{
        ...
        items_per_page: stocksStore.itemsPerPage
      }"
...
import type { StockFilters as StockFiltersType } from '@/stores/stocks'
...
function handleFiltersUpdate(updatedFilters: StockFiltersType) {
...
function handleItemsPerPageChange(itemsPerPage: number) {
  stocksStore.setItemsPerPage(itemsPerPage)
  stocksStore.fetchStocks()
}
